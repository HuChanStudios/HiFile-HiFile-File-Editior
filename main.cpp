	...
	if(bSave) {
		...
		if(GetSaveFileName(&ofn)) {
			if(!SaveFile(GetDlgItem(hwnd, IDC_MAIN_TEXT), szFileName)) {
				MessageBox(hwnd, "[错误]文件保存失败", "Error",MB_OK|MB_ICONEXCLAMATION);
				return FALSE;
			}
		}
	} else {
		...
		if(GetOpenFileName(&ofn)) {
			if(!LoadFile(GetDlgItem(hwnd, IDC_MAIN_TEXT), szFileName)) {
				MessageBox(hwnd, "[错误]文件加载失败", "Error",MB_OK|MB_ICONEXCLAMATION);
				return FALSE;
			}
		}
	}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,LPSTR lpCmdLine, int nCmdShow) {
	...
	if(!RegisterClassEx(&wc)) {
		MessageBox(0,"程序发生致命错误！\n - 具体原因：窗口注册失败\n - 详细错误原因：Window Register Error\n - 错误代码：0x1\n - 解决办法：重启程序","Error!",MB_ICONEXCLAMATION|MB_OK|MB_SYSTEMMODAL);
		return 0;
	}

	hwnd = CreateWindowEx(WS_EX_CLIENTEDGE,"WindowClass","文件编辑器",WS_OVERLAPPEDWINDOW,
		CW_USEDEFAULT,
		CW_USEDEFAULT,
		320,240,
